...
test "apply transition on mocked chain" {
    ...
    try ssz.hashTreeRoot(Sha256, types.BeamState, beam_state, &post_state_root, allocator);
...
test "genStateBlockHeader" {
    ...
    for (0..mock_chain.blocks.len) |i| {
        ...
        try ssz.hashTreeRoot(Sha256, types.BeamBlock, applied_block.message.block, &applied_block_root, allocator);

        const state_block_header = try beam_state.genStateBlockHeader(allocator);
        var state_block_header_root: types.Root = undefined;
        try ssz.hashTreeRoot(Sha256, types.BeamBlockHeader, state_block_header, &state_block_header_root, allocator);
